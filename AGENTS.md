...
## Назначение проекта
- `hse-lms-harvest` — локальный `uv`/Python-инструмент для легитимного сбора страниц HSE Smart LMS и выбранных модулей Netology из отдельных браузерных профилей пользователя.
...
- Netology обходится только в пределах точного module URL. `--open-netology-assignments` — отдельное узкое разрешение нажать «Приступить к заданию» для чтения полного условия. После клика дождись загруженного текста; «Отправить решение» и поля ответа не трогай. Без флага состояние задания не меняется. CAPTCHA проходит пользователь вручную.
...
## Формат дампа
- `manifest.json` — машинно читаемый источник правды после capture-level фильтров. В нём остаются полные URL полезных ссылок, кнопки, источники файлов, checksum и ошибки. `coverage` версии 1 явно передаёт область, подтверждённые страницы, остаток и причину остановки; полноту старым дампам при миграции не приписывать. Контракт потребителя и продолжения описан в README.
...
- Кэш файла переиспользовать после проверки локальной целостности и актуальности через HTTP-валидатор; при отказе HEAD использовать условный GET, а без достаточных сведений загрузить содержимое. Валидатор HTML не подтверждает отдельное вложение: его проверять независимо, сохраняя пригодный захват страницы.
...
- При смене правил извлечения/фильтрации обновлять `CAPTURE_VERSION` в `coverage.py`; при смене существенной настройки включать её в контракт пригодности. Изменение отображения не требует новой версии захвата.
- При продолжении `--resume-dump`/`--resume-latest` сначала перепроверять достижимость от текущего корня и пригодность прежних страниц; порция расходуется на неподтверждённую работу, а ошибочный сосед не блокирует весь остаток. Сохранять атомарный checkpoint и возвращать ненулевой exit для `partial`/`unknown`.
...
- `--page-concurrency` ограничен 1–3: корень обходится первым, независимые страницы можно читать одновременно, а записи вложений и итоговый порядок страниц сериализуются. Для Netology оставляй 1, пока не подтверждена безопасность одновременного открытия заданий. Проверяй полноту и повтор после прерывания на локальном сервере, а скорость — на полном живом курсе.
...
- `src/hse_lms_harvest/manifest.py` — чтение старых manifest, миграция дампов, page fingerprints и атомарная запись manifest.
- `src/hse_lms_harvest/coverage.py` — версия/условия захвата, область обхода, подтверждённые страницы и полнота результата.
...
- [README.md](README.md) описывает действующий контракт кэша, полноты и продолжения; меняй его вместе с кодом и затронутым потребителем Study.
